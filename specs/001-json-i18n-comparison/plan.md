...
**Primary Dependencies**: Vue 3.5.22, Vite (Rolldown variant 7.1.14), @vitejs/plugin-vue 6.0.1
**Storage**: Client-side only (browser memory, LocalStorage for tier preferences)
...
- **Status**: PASS
- **Validation**: Minimal dependencies (Vue + Vite only); no new packages needed for MVP
- **Action**: Use native browser APIs for file upload, JSON parsing, downloads
...
- [x] No additional dependencies introduced (uses only Vue 3, Vite, and native browser APIs)
...
│   └── useTierManager.js      # Manage tier limits, check key counts
├── utils/
